...
export class UsuarioService { 
    ...
    http: Http;
    headers: Headers;
    url_usuario: string = 'http://localhost:3000/api/usuarios'
  ...
  cadastrar(usuario: UsuarioComponent): Promise<any> {
      if(usuario._id){
          return this.http.put(this.url_usuario + '/' + usuario._id, JSON.stringify(usuario),
            { headers: this.headers })
            .toPromise()
                .then(res => res.json())
                .catch();
      } else {
          return this.http.post(this.url_usuario, JSON.stringify(usuario),
          { headers: this.headers })
          .toPromise()
            .then(res => res.json())
            .catch();
